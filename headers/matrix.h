...
inline Matrix mat_id(size_t n) {
  Matrix I(n, n, 0.0);
  for (size_t i = 0; i < n; i++) I(i,i) = 1.0;
  return I;
}

inline Matrix mat_mul(const Matrix& A, const Matrix& B){ //matrix multiplication
  Matrix C(A.rows(), B.cols(), 0.0);
  for (size_t i=0; i<A.rows(); i++){
    for (size_t j=0; j<A.cols(); j++){
      for (size_t k=0; k<B.cols(); k++){
        C(i,j) += A(i,k)*B(k,j); //C_{ij} = A_{ik}B_{kj}
      }
    }
  }
  return C;
...
inline Vector mat_vec_mul(const Matrix& A, const Vector& v){ //u_i = A_{ij}v_j
  Vector u(A.rows(), 0.0);
  for (size_t i=0; i<A.rows(); i++){
    for (size_t j=0; j<A.cols(); j++){
      u[i] += A(i,j)*v[j];
...
//LU decomposition will be needed for implicit solver
//first partial pivot after overwriting A with L below diag & U upper diag matrices
//
inline std::vector<int> mat_lu_factor(Matrix& A) {
  size_t n = A.rows();
  ...
  for (size_t k = 0; k < n; k++) {
    ...
    if (A(k,k) == 0.0) throw std::runtime_error("Singular block in LU");
    for (size_t i = k+1; i < n; i++) {
      A(i,k) /= A(k,k);
...
//LU*x = b using the factored matrix and pivot from mat_lu_factor
inline Vector mat_lu_solve(const Matrix& LU, const std::vector<int>& piv, const Vector& b) {
  size_t n = LU.rows();
  Vector x(n);
  for (size_t i = 0; i < n; i++) x[i] = b[piv[i]];  //apply pivot
  for (size_t i = 1; i < n; i++) //forward sub (L)
    for (size_t k = 0; k < i; k++)
      x[i] -= LU(i,k)*x[k];
  for (int i = n-1; i >= 0; i--) {//back sub (U)
